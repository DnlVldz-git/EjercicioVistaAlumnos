...
exports.create = (req, res) => {
  ...
  if (
    ...
    !req.body.tipoCalificacion &&
    !req.body.descEvaluacion &&
    !req.body.ordenCertificado &&
    !req.body.idStudent &&
    !req.body.creditos
  ) {
    res.status(400).send({
  ...
  StudentCareer.findAll({
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Ocurrio un error al recuperar estudiante por noControl.",
      });
    });
};
...
exports.deleteAll = (req, res) => {
  StudentCareer.destroy({
    ...
    .then((nums) => {
      res.send({
        message: `${nums} Estudinates detalle fueron eliminados con exito!`,
      });
    })
    .catch((err) => {
      res.status(500).send({
